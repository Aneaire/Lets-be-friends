...
function NotificationsPage() {
  ...
  if (!isSignedIn) {
    return <main className="notifications-page"><div className="notifications-empty"><Bell size={28} aria-hidden="true" /><h1 className="text-h1">Sign in to view notifications</h1><SignInButton mode="modal"><button className="btn btn-self">Sign in</button></SignInButton></div></main>
  }
  ...
  return (
    <main className="notifications-page">
      <header className="notifications-page-header">
        ...
        <button className="btn btn-neutral btn-sm" disabled={!notifications.some((item) => !item.readAt)} onClick={() => void markAllRead()}><CheckCheck size={16} aria-hidden="true" />Mark all as read</button>
      </header>
